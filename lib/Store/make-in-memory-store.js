...
import { md5, toNumber } from '../Utils/generics.js'
...
export const makeInMemoryStore = (config) => {
    ...
    const bind = (ev) => {
        ...
        ev.on('message-receipt.update', updates => {
            for (const { key, receipt } of updates) {
                ...
                if (msg) {
                    // Assuming updateMessageWithReceipt exists in utils
        ...
        ev.on('messages.reaction', (reactions) => {
            for (const { key, reaction } of reactions) {
                ...
                const msg = obj?.get(key.id)
                if (msg) {
                    // Assuming updateMessageWithReaction exists in utils
    ...
    const toJSON = () => {
        ...
        for (const jid in messages) {
            serializedMessages[jid] = messages[jid].all();
        }
        ...
        return {
            ...
            labels: labels.all ? labels.all() : labels,
    ...
    // --- FIX IS HERE ---
    const fromJSON = (json) => {
        // Handle both old (object) and new (array) formats for chats
        const chatsToLoad = Array.isArray(json.chats) ? json.chats : json.chats?.array || [];
        chats.upsert(...chatsToLoad);

        // Handle both old (object) and new (array) formats for labelAssociations
        const labelAssociationsToLoad = Array.isArray(json.labelAssociations) ? json.labelAssociations : json.labelAssociations?.array || [];
        labelAssociations.upsert(...labelAssociationsToLoad);
        ...
        for (const jid in json.messages) {
            const list = assertMessageList(jid);
            // Handle both old (object) and new (array) formats for messages
            const messagesToLoad = Array.isArray(json.messages[jid]) ? json.messages[jid] : json.messages[jid]?.array || [];
