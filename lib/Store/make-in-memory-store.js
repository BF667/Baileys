...
export const makeInMemoryStore = (config) => {
    ...
    const toJSON = () => {
        ...
        for (const jid in messages) {
            // Convert each message dictionary into a plain array for serialization
            serializedMessages[jid] = messages[jid].all();
        }

        return {
            // Convert KeyedDB instances into plain arrays
            chats: chats.all(),
            contacts: contacts,
            messages: serializedMessages,
            // Assuming ObjectRepository has a .all() method or is already a plain object
